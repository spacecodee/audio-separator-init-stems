...
AUDIO="${AUDIO:-${SEPARATE_AUDIO:-${DEFAULT_AUDIO:-/teamspace/studios/this_studio/audio/Audio04.wav}}}"
MODEL="${MODEL:-${SEPARATE_MODEL:-mel_roformer}}"
...
JOB_RESPONSE="$(curl -sS -X POST "$BASE/separate" \
  -F "file=@$AUDIO" \
  -F "model=$MODEL" \
